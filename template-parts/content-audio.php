<?php
/**
 * Audio post format — Eggxi `.ulockd-project-sm-thumb` SoundCloud / player header.
 *
 * Priority: ACF/meta URL → first content audio/iframe → featured image.
 *
 * @package Eggxi
 */
...
$audio         = eggxi_get_post_audio_embed();
$has_audio     = ( '' !== $audio['html'] );
$body_content  = '';

if ( $has_audio ) {
	$filtered = ( 'content' === $audio['source'] && $audio['content'] )
		? $audio['content']
		: apply_filters( 'the_content', get_the_content() );

	$body_content = eggxi_strip_video_embed_from_content( $filtered, $audio['html'] );
...
?>
<article id="post-<?php the_ID(); ?>" <?php post_class( 'blog_single_post_wrap format-audio' ); ?>>
	<div class="row">
		<div class="col-lg-12">
			<?php if ( $has_audio ) : ?>
				<div class="ulockd-project-sm-thumb responsive-audio-container ulockd-mb20">
					<?php echo wp_kses( $audio['html'], eggxi_audio_embed_allowed_html() ); ?>
				</div>
